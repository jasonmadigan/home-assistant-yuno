...
This fork of [omgapuppy/home-assistant-yuno](https://github.com/omgapuppy/home-assistant-yuno)
adds account login, automatic session renewal, and a helper for users of the
original integration. The Energy Dashboard statistics and sensors come from the
upstream project. This is a custom integration, not a Supervisor add-on, and uses
Yuno's undocumented mobile API. It is not affiliated with Yuno Energy.
...
## Helper for the original integration

If you prefer to keep the original integration, run this helper from a checkout
...
requests. It uses the same encryption and signing code as this fork.
...
The helper only outputs configuration values. This fork's Home Assistant flow
...
- **Unexpected response:** Yuno may have changed its undocumented API. Check for
  an updated version of this fork. Do not attach raw responses or setup values to
  public issues.
